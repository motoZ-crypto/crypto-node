...
`--miner` sets the reward address and exposes the mining RPC so external miners can scan off the node and submit seals. Pull the current task with `mining_getTask` or subscribe to `mining_subscribeTask` for a fresh task pushed every second, then return a found seal with `mining_submitSeal`. Add `--node-miner` to also run the in-process scan loop. Drop it to leave block authoring entirely to external miners.
